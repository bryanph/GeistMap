...
exports.find = function(req, res, next){
  ...
  req.app.db.models.AdminGroup.pagedFind({
    ...
    if (req.xhr) {
      ...
      results.filters = req.query;
      let template = req.app.utils.loadTemplate('admin/admin-groups/index.jade')
  ...
  req.app.db.models.AdminGroup.findById(req.params.id).exec(function(err, adminGroup) {
    ...
    if (req.xhr) {
      res.send(adminGroup);
        ...
        let template = req.app.utils.loadTemplate('admin/admin-groups/details.jade')
        res.write(template({ data: { record: escape(JSON.stringify(adminGroup)) } }))
        res.end()
    }
  });
};
