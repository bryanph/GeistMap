...
exports.find = function(req, res, next){
  ...
  req.app.db.models.User.pagedFind({
    ...
    if (req.xhr) {
      ...
      results.filters = req.query;
      let template = req.app.utils.loadTemplate('admin/users/index.jade')
      res.write(template({ data: { results: JSON.stringify(results) } }))
      res.end()
    }
  });
};
  ...
  req.app.db.models.User.findById(req.params.id).populate('roles.admin', 'name.full').populate('roles.account', 'name.full').exec(function(err, user) {
    ...
    if (req.xhr) {
      res.send(user);
        ...
        let template = req.app.utils.loadTemplate('admin/users/details.jade')
        res.write(template({ data: { record: escape(JSON.stringify(user)) } }))
        res.end()
    }
  });
};
