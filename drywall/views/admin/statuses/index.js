...
exports.find = function(req, res, next){
  ...
  req.app.db.models.Status.pagedFind({
    ...
    if (req.xhr) {
      ...
      let template = req.app.utils.loadTemplate('admin/statuses/index.jade')
  ...
  req.app.db.models.Status.findById(req.params.id).exec(function(err, status) {
    ...
    if (req.xhr) {
      res.send(status);
        ...
        let template = req.app.utils.loadTemplate('admin/statuses/details.jade')
        res.write(template({ data: { record: escape(JSON.stringify(status)) } }))
        res.end()
    }
  });
};
