...
export default function onDropBlock({ handleDefaultData }) {
    return function onDropBlockInner(selection, dataTransfer, isInternal, { getEditorState, setEditorState }) {
        ...
        if (data[0] === DRAFTJS_BLOCK_KEY) {
            ...
            const editorStateAfterInsert = addBlock(editorState, selection, block.getType(), contentState.getEntity(block.getEntityAt(0)).data);
        ...
        if (data[0] === DRAFTJS_BLOCK_TYPE) {
            ...
            // Get content, selection, block
            const editorStateAfterInsert = addBlock(editorState, selection, blockType, handleDefaultData ? handleDefaultData(blockType) : {});
